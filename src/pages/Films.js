...
const Films = () => {
    ...
    const fetchedFilms       = useSelector((state) => state.filmsReducer.fetchedFilms);
    const httpIsSending      = useSelector((state) => state.filmsReducer.sending);
    const httpError          = useSelector((state) => state.filmsReducer.error);
    const httpAlreadyFetched = useSelector((state) => state.filmsReducer.atLeastOneFetch);
    ...
    // retry
    const retryHandler = () => { setRetry(!retry) };
    ...
    return (
        <Fragment>
            <Header hasBack={0} />
            <main className="main">
                {!httpIsSending && !httpError && httpAlreadyFetched && <FilmsTable films={fetchedFilms} />}
